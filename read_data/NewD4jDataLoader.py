...
class NewD4jDataLoader(DataLoader):
    # self.concrete_column, self.columnmap, self.fault_line
    ...
    def _load_fault_line(self):
        # 'D:\\university_study\\科研\\Diffusion_Model   \\d4j\\buggy-lines\\Chart-1.buggy.lines'
        # fault_dir = os.path.join(self.base_dir,"d4j", "buggy-lines", self.program + "-" + str(self.bug_id) + ".buggy.lines")
        fault_dir = os.path.join(self.temp_path, 
                "d4j",
                "buggy-lines",
                self.program + "-" + str(self.bug_id) + ".buggy.lines")
        ...
        self.fault_line = [int(i) for i in self.fault_line]
        print("fault line: ", self.fault_line)
...
if __name__ == '__main__':
    ...
    project_dir += "\\data"
    print(project_dir)
    data_loader = NewD4jDataLoader(project_dir, "Chart", 1)
    data_loader.load()
